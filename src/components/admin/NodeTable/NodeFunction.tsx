...
import { AGENT_INSTALL_RAW_BASE } from "@/lib/repoSources";
import {
  Button,
  Checkbox,
  Dialog,
...
export function ActionsCell({ row }: { row: Row<z.infer<typeof schema>> }) {
  ...
  const generateCommand = () => {
    ...
    switch (selectedPlatform) {
      case "linux":
        finalCommand =
          `wget -qO- ${AGENT_INSTALL_RAW_BASE}/install.sh | sudo bash -s -- ` +
          quoteShellArgs(args);
        break;
      case "windows":
        finalCommand =
          `powershell.exe -NoProfile -ExecutionPolicy Bypass -Command ` +
          `"iwr '${AGENT_INSTALL_RAW_BASE}/install.ps1'` +
      ...
      case "macos":
        finalCommand =
          `zsh <(curl -sL ${AGENT_INSTALL_RAW_BASE}/install.sh) ` +
          quoteShellArgs(args);
        break;
    }
    return finalCommand;
